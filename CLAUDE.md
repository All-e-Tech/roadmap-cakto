...
Protótipo funcional em validação, usado pelos PMs das 5 squads e pelo GPM. Single-page app sem build (HTML/CSS + Preact e htm via CDN) com quatro views: Roadmap (tabela), Visão geral (Gantt), Kanban de iniciativas, Squads & sprints. No ar em `https://roadmap-cakto.vercel.app` (Vercel + Supabase, atrás de senha compartilhada) desde 15/09/2026.
...
- **Acesso a dados só por `data.js`.** Nenhum componente lê `data.quarters[…].items` ou `data.iniciativas` direto — usa funções de `data.js`. Motivo: a remodelagem do 4b (iniciativa dona do item e da categoria) deve ficar contida em um arquivo.
...
- Reabrir o vínculo Iniciativa → Roadmap sem decisão nova. Está **fechado** em 17/09/2026 (`SPEC.md` A.1 e §3, `docs/decisions.md`): 1 iniciativa = N itens, todo item tem uma iniciativa, a categoria é da iniciativa, não existe subcategoria nem sub-item.
...
4. **Backend — concluído em 15/09/2026.** Vercel + Supabase (Postgres), conforme `SPEC.md` §2.1: `api/board.js` com adaptador Supabase e fallback em memória, `sync.js` conforme §9, deploy conforme §10. Fechamento e o que ficou de fora em `docs/divergences.md`. Para rodar local, `node tools/dev-server.mjs` (o `python -m http.server` não roda a função).
4b. **1 iniciativa = N itens — concluído em 18/09/2026.** Modelo em `SPEC.md` §3 e A.1; racional em `docs/decisions.md`. Três entregas: (1) o vínculo — `ini` no item, categoria na iniciativa, fim da subcategoria, derivados, criação nos dois sentidos, os quatro movimentos e o par extrair/juntar; (2) arquivado e descartado (`arq`, §4.3) com a seção de consulta em Squads & sprints; (3) colunas Iniciativas e Backlog Priorizado e PM/TL padrão por squad. `normalize()` migra boards antigos ao carregar; `node tools/teste-modelo.mjs` cobre migração e invariantes. Para zerar o board: **Carregar** com `docs/board-vazio.json`. Fora do escopo, para quando o GPM pedir: arquivar ou descartar um **item** isolado, sem mexer na iniciativa.
