...
4b. **1 iniciativa = N itens — implementação.** Modelo fechado em 17/09/2026 (`SPEC.md` §3 e A.1; racional em `docs/decisions.md`). Três entregas, uma por pedido: (1) o vínculo — `ini` no item, categoria na iniciativa, fim da subcategoria, derivados, criação nos dois sentidos, os quatro movimentos e o par extrair/juntar; (2) arquivado e descartado (`arq`, §4.3) com a seção de consulta em Squads & sprints; (3) rótulos do Kanban (colunas Iniciativas e Backlog Priorizado) e PM/TL padrão por squad. Mudança de modelo: `normalize()` migra ao carregar; o storage é documento. O board em produção renasce **vazio** — os dados de hoje são de teste e serão descartados.
